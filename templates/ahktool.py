...
def render_ahk_script():
    if doc['div_hotkey_setting'].innerHTML!="": #若設定組合鍵勾選區塊已經被BRYTHON生成，才執行以下動作
        #獲取組合鍵設定模式
        hotkey_setting_mode=doc['div_hotkey_setting'].select("input[type=radio]:checked")[0].value
        #若組合鍵設定模式不為自訂，則輸出一般的組合鍵符號
        if hotkey_setting_mode!="custom":
            #獲取所有勾選的inputCheckbox組合鍵元素列表
            inputCheckbox_func_key_elt_list=doc.select("#div_hotkey_setting input.function_key[type='checkbox']")
            #獲取所有勾選的inputCheckbox前綴功能元素列表
            inputCheckbox_prefix_function_elt_list=doc.select("#div_hotkey_setting input.prefix_function[type='checkbox']")

            #獲取已勾選前綴符號
            prefix_symbol="".join(checkbox.symbol if checkbox.checked else "" for checkbox in inputCheckbox_prefix_function_elt_list)
            #獲取已勾選組合鍵符號
            function_key_symbol="".join(checkbox.symbol if checkbox.checked else "" for checkbox in inputCheckbox_func_key_elt_list)
            #獲取一般鍵盤符號
            normal_key_symbol=doc['div_hotkey_setting_area'].select("select.normal_key")[0].value
        ...
        #若組合鍵設定模式為自訂，則輸出匹配型按鍵符號
            ...
            select_elt_normal_key_list=doc['div_hotkey_setting_area'].select("select.normal_key")
            #若組自訂合鍵設定模式區塊加載完成
            if len(select_elt_normal_key_list)==2:
                ...
                prefix_symbol="".join(checkbox.symbol if checkbox.checked else "" for checkbox in inputCheckbox_prefix_function_elt_list)
                #獲取自訂雙按鍵符號
                key_A,key_B=[select_elt.value for select_elt in doc['div_hotkey_setting_area'].select("select.normal_key")]
           ...
           #獲取SELECT組合鍵功能元素
            ...
            if select_elt.value in ["開啟資料夾","開啟網頁","執行程式"]:
                ...
                if select_elt.value=="開啟網頁":
                    ahk_exec_script=ahk_exec_script.replace("%","`%").replace(",","`,")
                ...
                ahk_key_script=":"
                ahk_key_script+="".join([(inputCheckbox_elt.symbol if inputCheckbox_elt.checked else inputCheckbox_elt.nosymbol) for inputCheckbox_elt in inputCheckbox_elt_list[:-1] ])
                ...
                input_text=(doc['input_inserted_text'].value if doc['input_inserted_text'].value else doc['input_inserted_text'].placeholder)
                #若選擇使用剪貼簿
                ...
                input_radio_checked_elt=div_func_conent_elt.select("input[type='radio']:checked")[0]
                ...
                system_action_force=doc["DIV_func_conent"].select("input[type='checkbox']")[0].checked
                if system_action=="lock":
                    ...
                    UndisableInputElt(doc["DIV_func_conent"].select("input[type='checkbox']")[0])
                    ahk_exec_script="Shutdown, {}".format(2+4*system_action_force)
                else:
                    ahk_exec_script=""
                ...
                x_pos,y_pos,ClickCount,Speed=[inputNumberElt.value for inputNumberElt in doc["DIV_func_conent"].select("input[type='number']")]
                coord_mode=doc["DIV_func_conent"].select("input[type='radio']:checked")[0].value
                coord_mode=coord_mode[0].upper()+coord_mode[1:]
                Return=("true" if doc["DIV_func_conent"].select("input[type='checkbox']")[0].checked else "false")
                ahk_exec_script=f"ClickPosition({x_pos}, {y_pos}, {ClickCount}, {Speed}, \"{coord_mode}\", {Return})"
            
            elif select_elt.value=="點擊畫面中的圖片":
                ImageFilePath=doc["DIV_func_conent"].select("input[type='text']")[0].value
                ClickCount,Speed=[inputNumberElt.value for inputNumberElt in doc["DIV_func_conent"].select("input[type='number']")]
                Return=("true" if doc["DIV_func_conent"].select("input[type='checkbox']")[0].checked else "false")
                ShowError=("true" if doc["DIV_func_conent"].select("input[type='checkbox']")[1].checked else "false")
                ahk_exec_script=f"ClickPicture(\"{ImageFilePath}\", {ClickCount}, {Speed},{Return},{ShowError})"

            elif select_elt.value=="AHK測試工具":
                ahk_test_tool_action=doc["DIV_func_conent"].select("input[type='radio']:checked")[0].value
                selected_show_msg=doc["DIV_func_conent"].select("select")[0].value
                input_elt=doc["DIV_func_conent"].select("input[type='text']")[0]
                input_elt_value=(input_elt.value if input_elt.value else input_elt.placeholder)
                if ahk_test_tool_action=="msgbox":
                    ahk_exec_script=ShowMsgByMsgbox_ahkCommand(input_elt_value,selected_show_msg)
                elif ahk_test_tool_action=="tooltip":
                    ahk_exec_script=ShowMsgByTooltip_ahkCommand(input_elt_value,selected_show_msg)
            ...
            ##
                ...
                select_elt=doc["DIV_func_conent"].select("select")[0]
                input_cssSelector_elt=doc["DIV_func_conent"].select("input.css_selector")[0]
                if doc["DIV_func_conent"].select("input.input_text"):
                    input_value=doc["DIV_func_conent"].select("input.input_text")[0].value
                elif doc["DIV_func_conent"].select("input.selected_index"):
                    input_value=doc["DIV_func_conent"].select("input.selected_index")[0].value
                else:
                    input_value=0
                input_delayTime=doc["DIV_func_conent"].select("input.delay_time")[0].value
    ...
    ahk_function_srcipt_output_text="".join([FUNCTION_DICT[function_name] for function_name in ahk_related_function_list])
    ...
    #若組合鍵AHK語法包含函式，就不斷追加函式內容語法中包含的函式內容
        ...
        while True:
            ...
            ahkFuntionOnly_related_function_list=RelateFunctionList(ahk_function_srcipt_output_text)
            #若全部函式都已經包含在組合鍵AHK語法包含的函式內，就中止迴圈
            if set(ahkFuntionOnly_related_function_list) <= set(ahk_related_function_list):
                break
            #若還有
            else:
                new_function_list=list(set(ahkFuntionOnly_related_function_list)-set(ahk_related_function_list))
                ahk_related_function_list.extend(new_function_list)
                ahk_function_srcipt_output_text+="".join([FUNCTION_DICT[function_name] for function_name in new_function_list])
        #開啟函式頁籤
...
def Div_hotkey_setting():
    ...
    def DIV_elt_Expanded_prefixSymbol_setting():
        ...
        def checkbox_onClick(ev):
            ...
            input_elt=inputCheckbox_elt.parent.parent.select('input[type="text"]')[0]
        ...
        #設置組合鍵前綴展開DIV子區塊
        div_elt_prefixSymbol_setting_area=DIV(id="div_elt_prefixSymbol_setting_area")
        div_elt_prefixSymbol_setting_area<=A_INPUTCheckbox("保留預設按鍵功能",attr_dict={"symbol":"~"},Class="prefix_function")+SPAN("　")
        div_elt_prefixSymbol_setting_area<=A_INPUTCheckbox("避免自我觸發",attr_dict={"symbol":"$"},Class="prefix_function")+SPAN("　")
        div_elt_prefixSymbol_setting_area<=A_INPUTCheckbox("可在其他組合鍵觸發",attr_dict={"symbol":"*"},Class="prefix_function")+BR()
        a_inputCheckbox_elt=A_INPUTCheckbox("僅在標題含有該文字的視窗下執行: ",id="ifWinTitle_CheckboxElt")
        a_inputCheckbox_elt.select("input")[0].bind("click",checkbox_onClick)
        div_elt_prefixSymbol_setting_area<=a_inputCheckbox_elt
        div_elt_prefixSymbol_setting_area<=INPUT(type="text",id="ifWinTitle_InputElt",size="30",disabled=True)
    ...
    def DIV_elt_simple_setting():
        ...
        #輸出勾選功能鍵
        div_elt<=A_INPUTCheckbox("Ctrl",attr_dict={"symbol":"^"},Class="function_key")+SPAN(" ")
        div_elt<=A_INPUTCheckbox("Shift",attr_dict={"symbol":"+"},Class="function_key")+SPAN(" ")
        div_elt<=A_INPUTCheckbox("Alt",attr_dict={"symbol":"!"},Class="function_key")+SPAN(" ")
        div_elt<=A_INPUTCheckbox("Win",attr_dict={"symbol":"#"},Class="function_key")+SPAN(" ")
        #加上「+」符號
        div_elt<=SPAN("+ ")
        #匹配一般按鍵
    ...
    def DIV_elt_advanced_setting():
        ...
        tr_left_function_key<=TD(A_INPUTCheckbox("LCtrl",attr_dict={"symbol":"<^"},Class="function_key"))
        tr_left_function_key<=TD(A_INPUTCheckbox("LShift",attr_dict={"symbol":"<+"},Class="function_key"))
        tr_left_function_key<=TD(A_INPUTCheckbox("LAlt",attr_dict={"symbol":"<!"},Class="function_key"))
        tr_left_function_key<=TD(A_INPUTCheckbox("LWin",attr_dict={"symbol":"<#"},Class="function_key"))
        ...
        tr_right_function_key<=TD(A_INPUTCheckbox("RCtrl",attr_dict={"symbol":">^"},Class="function_key"))
        tr_right_function_key<=TD(A_INPUTCheckbox("RShift",attr_dict={"symbol":">+"},Class="function_key"))
        tr_right_function_key<=TD(A_INPUTCheckbox("RAlt",attr_dict={"symbol":">!"},Class="function_key"))
        tr_right_function_key<=TD(A_INPUTCheckbox("RWin",attr_dict={"symbol":">#"},Class="function_key"))
    ...
    def DIV_elt_custom_setting():
        ...
        #排版
        div_elt<=select_elt_hotkey_A+SPAN(" + ")+select_elt_hotkey_B+SPAN("　")
    ...
    def ChangeHotkeySettingModel(ev):
        ...
        remain_keyFuntion_inputCheckbox_elt=doc['div_elt_prefixSymbol_setting_area'].select('input[type="checkbox"]')[0]
        #若為簡易模式
    ...
    div_hotkey_setting<=A_INPUTRadio("簡易",checked=True,name="hotkey_setting_mode",value="simple").bind("input",ChangeHotkeySettingModel)+SPAN(" ")
    div_hotkey_setting<=A_INPUTRadio("進階",checked=False,name="hotkey_setting_mode",value="advanced").bind("input",ChangeHotkeySettingModel)+SPAN(" ")
    div_hotkey_setting<=A_INPUTRadio("自訂",checked=False,name="hotkey_setting_mode",value="custom").bind("input",ChangeHotkeySettingModel)
...
def DIV_function():
    ...
    def DIV_func_conent(func_name):
        ...
        if func_name=="開啟檔案":
            ...
            div_elt<=SPAN("資料夾路徑: ")+INPUT(type="text",size="30",placeholder="C:\\")
        elif func_name=="開啟網頁":
            div_elt<=SPAN("網址: ")
            div_elt<=INPUT(type="text",size="30",placeholder="www.google.com")
        elif func_name=="進入滑鼠選取的網址":
            pass
        elif func_name=="搜尋滑鼠選取的關鍵字":##待追加
            select_elt=SELECT(style={"margin-top":"10px"},id="select_searchWebSite")
            ...
            span_websiteName_elt<=SPAN(" 網站名稱(可選):")+INPUT(type="text",id="input_website_name")

            div_elt<=SPAN("進行 ")+select_elt
            div_elt<=span_websiteName_elt
            div_elt<=BR()+BR()
            div_elt<=A_INPUTCheckbox("若無選取文字，則跳出搜尋框",checked=True,id="inputCheck_showInputBox")
            div_elt<=HR()
            div_elt<=SPAN("關鍵字之前的網址: ")
            input_elt_A=INPUT(value="https://www.google.com.tw/search?q=",type="text",size="40",disabled=True,id="input_A_url")
            div_elt<=input_elt_A+BR()
            div_elt<=SPAN("關鍵字之後的網址: ")
            input_elt_B=INPUT(value="",type="text",size="40",disabled=True,id="input_B_url")
            div_elt<=input_elt_B
            ...
            div_elt<=SPAN("座標　X:")+INPUT(type="number",value="100",min="0",style={"width":"60px"})+SPAN(" Y: ")+INPUT(type="number",value=200,step=1,style={"width":"60px"})+BR()
            div_elt<=TABLE(
                TR(
                    TD(SPAN("以",style={"margin-left":"-3px"}),rowspan="3")
                    +TD(A_INPUTRadio("屏幕",checked=True,name="coordinate_item",value="screen"))
                    +TD(SPAN("的左上角作為座標原點"),rowspan="3")
                )
                +TR(TD(A_INPUTRadio("當前視窗",checked=False,name="coordinate_item",value="window")))
                +TR(TD(A_INPUTRadio("滑鼠目前位置",checked=False,name="coordinate_item",value="relative")))
                ,style={"margin-top":"10px"}
            )
            div_elt<=SPAN("點擊")+INPUT(type="number",value=1,min=0,step=1,style={"width":"30px"})+SPAN("次　")
            div_elt<=SPAN("移動延遲")+INPUT(type="number",value=0,min=0,max=10,step=1,style={"width":"30px"}).bind("input",inputTextChangeRange).bind("input",inputTextOrRangeChangeSpan)+INPUT(type="range",value=0,max=10,min=0,style={"width":"80px","position":"relative","top":"6px"}).bind("input",inputRangeChangeText).bind("input",inputTextOrRangeChangeSpan)+SPAN("立即")+BR()

            a_INPUTCheckbox=A_INPUTCheckbox("點擊後返回原位置",checked=True)
            a_INPUTCheckbox.select('input[type="checkbox"]')[0].style={'margin-top':'12px'}
            ...
            div_elt<=SPAN("文字: ")+INPUT(type="text",size="30",placeholder="123",id="input_inserted_text")+BR()
            form_elt=FORM()
            ...
            div_elt<=SPAN("輸入縮寫: ")+INPUT(type="text",size="10",placeholder="btw").bind("focusin",ClearPlaceholder)+SPAN("時 ,")+BR()
            ...
            form_elt<=A_INPUTRadio("提高 ",checked=True,name="volumn",value="up")+INPUT(value="2",type="number",style=input_number_style_dict).bind("focus",ClickPreviousSelectEltWhenInputChange)+SPAN(" %")+BR()
            form_elt<=A_INPUTRadio("降低 ",name="volumn",value="down")+INPUT(value="2",type="number",max="50",min="0",style=input_number_style_dict).bind("focus",ClickPreviousSelectEltWhenInputChange)+SPAN(" %")+BR()
            form_elt<=A_INPUTRadio("靜音/取消靜音",name="volumn",value="mute")+BR()
            form_elt<=A_INPUTRadio("變更為 ",name="volumn",value="set")+INPUT(value="50",step="1",type="range",max="100",min="0").bind("input",ChangeVolumeValue).bind("focus",ClickPreviousSelectEltWhenInputChange)+SPAN("50")+SPAN(" %")+BR()
            div_elt<=form_elt
            ...
            div_elt<=A_INPUTRadio("跳出視窗　",name="ahkTestTool_action",value="msgbox",checked=True)
            div_elt<=A_INPUTRadio("跳出提示",name="ahkTestTool_action",value="tooltip")+BR()
            ...
            def MsgContent():
                ...
                div_elt<=select_elt+INPUT(type="text",size="20",placeholder=default_msg_str).bind("focus",ClearPlaceholder).bind("focusout",RecoverPlaceholder)
                return div_elt
            ...
            def DIV_InputWebEltAction():
                div_elt=DIV()
                div_elt<=DIV_SAPN_INPUT_targetWebEltCSS()
                div_elt<=SPAN("輸入文字 ")+INPUT(style={"margin-top":"10px"},Class="input_text").bind("input",RenderAhkIfInputChange)
                div_elt<=DIV_settingDelay()
                return div_elt
        ...
        inputAndSelect_elt_in_input_key_area_list=div_hotkey_setting.select("input")+div_hotkey_setting.select("select")
...
def DIV_SciptTabs():
    ...
    div_elt<=DIV_Tab("關聯函式",id="function_div_page_tag",correspond_div_page_id="ahk_function_srcipt_output")
    div_elt<=DIV_Tab("所有函式",id="all_function_div_page_tag",correspond_div_page_id="all_function_srcipt_output")
    return div_elt
